...
Ce service est disponible gratuitement pour les clients de longue durée d’AEM Forms 6.5 et d’AEM Forms 6.4 sur site et les clients d’entreprise du service géré Adobe. Vous pouvez contacter l’équipe commerciale d’Adobe ou votre représentant Adobe pour demander l’accès au service.
...
* Une instance d’auteur AEM 6.5 ou AEM 6.4 opérationnelle avec la dernière version d’AEM Service Pack
...
1. [Téléchargement et installation d’AEM 6.5 ou d’AEM 6.4](#aemquickstart)
...
1. [Création de thèmes et de modèles personnalisés](#referencepackage)

### Téléchargement et installation d’AEM 6.5 ou d’AEM 6.4 {#aemquickstart}


Le service de conversion automatisée des formulaires s’exécute sur l’instance d’auteur AEM. Vous avez besoin d’AEM 6.5 ou d’AEM 6.4 pour configurer une instance d’auteur AEM. Si AEM n’est pas en cours d’exécution, téléchargez-le à partir des emplacements suivants :

* Si vous êtes déjà client AEM, téléchargez AEM 6.5 ou AEM 6.4 depuis le site Web [d’achat de licences](http://licensing.adobe.com)Adobe.

* Si vous êtes partenaire Adobe, utilisez le programme [de formation des partenaires](https://adobe.allegiancetech.com/cgi-bin/qwebcorporate.dll?idx=82357Q) Adobe pour demander AEM 6.5 ou AEM 6.4.
...
Téléchargez et installez la dernière version d’AEM Service Pack. Pour obtenir des instructions détaillées, reportez-vous aux Notes [de mise à jour d’](https://helpx.adobe.com/experience-manager/6-5/release-notes/sp-release-notes.html) AEM 6.5 Service Pack ou aux Notes [de mise à jour d’](https://helpx.adobe.com/experience-manager/6-4/release-notes/sp-release-notes.html)AEM 6.4 Service Pack.

### Download and install AEM Forms add-on package  {#downloadaemformsaddon}

Une instance AEM contient les fonctionnalités de base des formulaires. Le service de conversion nécessite des fonctionnalités complètes d’AEM Forms. Téléchargez et installez le module complémentaire AEM Forms pour bénéficier de toutes les fonctionnalités d’AEM Forms. Le package est requis pour configurer et exécuter le service de conversion. Pour des instructions détaillées, voir [Installation et configuration des fonctionnalités de capture de données.](https://helpx.adobe.com/experience-manager/6-5/forms/using/installing-configuring-aem-forms-osgi.html)

>[!NOTE]
> Si vous utilisez déjà le service de conversion automatisée des formulaires, installez le dernier module complémentaire AEM Forms pour continuer à utiliser ce service. Le package connecteur est fusionné dans le package du module complémentaire AEM Forms. Le module connecteur supplémentaire n&#39;est plus requis.
> Assurez-vous d’effectuer les configurations post-installation obligatoires après l’installation du module complémentaire.


### Création de thèmes et de modèles personnalisés {#referencepackage}

Si vous démarrez AEM en mode [de](https://helpx.adobe.com/experience-manager/6-5/sites/administering/using/production-ready.html) production (mode d’exécution nosamplecontent), les packages de référence ne sont pas installés. Les packages de référence contiennent des exemples de thèmes et de modèles. Le service de conversion automatisée des formulaires requiert au moins un thème et un modèle pour convertir un formulaire PDF en formulaire adaptatif. Créez un thème et un modèle personnalisés de votre propre configuration [de](#configure-the-cloud-service) service point et de votre propre configuration pour utiliser des modèles et des thèmes personnalisés avant d’utiliser le service.
...
Une fois qu’Adobe a activé l’accès pour votre entreprise et accordé les privilèges requis à l’administrateur, l’administrateur peut se connecter à la Console d’administration (instructions détaillées ci-dessous), créer un profil et ajouter des développeurs au profil. Les développeurs peuvent connecter une instance locale d’AEM Forms au service de conversion automatisée des formulaires sur Adobe Cloud.

Les développeurs sont des membres de votre organisation désignés pour exécuter le service de conversion. Seuls les développeurs qui sont ajoutés au profil du service de conversion automatisée des formulaires Adobe sont autorisés à utiliser le service de conversion automatisée des formulaires. Pour créer un profil et y ajouter des développeurs, procédez comme suit :
...
1. Spécifiez **[!UICONTROL Name]**, **[!UICONTROL Display Name]** et **[!UICONTROL Description]** le profil. Cliquez sur **[!UICONTROL Done]**. Un profil est créé.

   ![Spécifiez les détails du nouveau profil.](assets/create-new-profile-details.png)

1. Ajoutez le développeur au profil. Pour ajouter les développeurs :
   ...
   1. Sélectionnez les profils de produit. Appuyer **[!UICONTROL Save]**.
...
Une fois qu’un administrateur ajoute des développeurs au profil d’E/S Adobe, les développeurs en sont avertis par courrier électronique. Après avoir reçu le courrier électronique, les développeurs peuvent continuer à [connecter une instance locale d’AEM Forms au service de conversion automatisée des formulaires sur Adobe Cloud](#connectafcadobeio).
...
* [Ajouter un utilisateur au groupe d’utilisateurs de formulaires](#adduserstousergroup)
...
1. Ouvrez la configuration du Service de messagerie Day CQ. Spécifiez une valeur pour les champs **[!UICONTROL SMTP server host name]**, **[!UICONTROL SMTP server port]** et **[!UICONTROL From address]** . Cliquez sur **[!UICONTROL Save]**.
   ...
   Vous pouvez contacter votre fournisseur de services de messagerie ou votre administrateur informatique pour plus d’informations sur le nom d’hôte et le port du serveur SMTP. Vous pouvez utiliser n’importe quelle adresse électronique valide dans le champ De. Par exemple, notification@example.com ou donotreply@example.com.
...
1. Indiquez une adresse électronique dans le **[!UICONTROL Email]** champ et appuyez sur **[!UICONTROL Save]**. Les courriers électroniques sont envoyés à l’adresse électronique spécifiée lorsque la conversion a réussi ou échoué.
1. Tap the **Groups** tab. Dans l’onglet Sélectionner un groupe, saisissez et sélectionnez le groupe **forms-utilisateurs** . Tap **Save &amp; Close**. L’utilisateur est désormais membre du groupe des utilisateurs de formulaires.
...
1. Sélectionnez le profil créé lors de l’ [octroi d’un accès aux développeurs de votre organisation](#adduseranddevs) et appuyez sur **[!UICONTROL Create Integration]**. L’integration est créée.
1. Appuyez sur **[!UICONTROL Continue to integration details]** pour afficher les informations d’intégration. La page contient la clé d’API, la clé secrète client et d’autres informations requises pour connecter votre instance locale AEM au service de conversion automatisée des formulaires. Les informations de la page sont utilisées pour créer la configuration IMS sur votre ordinateur local.
...
1. Sur la **[!UICONTROL Create Automated Forms Conversion Configuration]** page, spécifiez la valeur des champs suivants et appuyez sur **[!UICONTROL Next]**.
   ...
   | Champ | Description |
   |--- |--- |
   | Titre | Titre unique de la configuration. Le titre s’affiche dans l’interface utilisateur utilisée pour lancer la conversion. |
   | Nom | Nom unique de la configuration. La configuration est enregistrée dans le référentiel CRX avec le nom spécifié. Le nom peut être identique au titre. |
   ...
   | Meta-model personnalisé | Chemin du fichier .schema.json du méta-modèle personnalisé. |
...
1. Dans l’ **[!UICONTROL Advanced]** onglet de la **[!UICONTROL Create Automated Forms Conversion Configuration]** page, spécifiez la valeur du champ suivant :
   ...
   <td>Sélectionnez l’option permettant de générer automatiquement le document d’enregistrement pour les formulaires convertis. Cette option est uniquement disponible pour les formulaires XFA (formulaires XDP et PDF). Lorsque vous activez cette option, après avoir envoyé un formulaire, vous pouvez permettre à vos clients de conserver un enregistrement, sous forme imprimée ou sous forme de document, des informations qu’ils ont renseignées dans le formulaire pour référence ultérieure. On parle ici de document d’enregistrement.</td>
   </tr>
   <tr>
...
1. Appuyer **[!UICONTROL Create]**.  La configuration du cloud est créée. Votre instance AEM Forms est prête à commencer à convertir des formulaires hérités en formulaires adaptatifs.
